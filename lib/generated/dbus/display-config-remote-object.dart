...
class OrgGnomeMutterDisplayConfigMonitorsChanged extends DBusSignal {
  OrgGnomeMutterDisplayConfigMonitorsChanged(DBusSignal signal)
      : super(
...
class OrgGnomeMutterDisplayConfig extends DBusRemoteObject {
  ...
  OrgGnomeMutterDisplayConfig(
    ...
    monitorsChanged = DBusRemoteObjectSignalStream(
            object: this,
            interface: 'org.gnome.Mutter.DisplayConfig',
            name: 'MonitorsChanged',
            signature: DBusSignature(''))
        .asBroadcastStream()
        .map((signal) => OrgGnomeMutterDisplayConfigMonitorsChanged(signal));
  }
  ...
  Future<void> setPowerSaveMode(int value) async {
    await setProperty(
        'org.gnome.Mutter.DisplayConfig', 'PowerSaveMode', DBusInt32(value));
  }
  ...
  Future<bool> getApplyMonitorsConfigAllowed() async {
    var value = await getProperty(
        'org.gnome.Mutter.DisplayConfig', 'ApplyMonitorsConfigAllowed',
        signature: DBusSignature('b'));
    return (value as DBusBoolean).value;
  }

  /// Invokes org.gnome.Mutter.DisplayConfig.GetResources()
  Future<List<DBusValue>> callGetResources(
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod(
        'org.gnome.Mutter.DisplayConfig', 'GetResources', [],
        replySignature:
            DBusSignature('ua(uxiiiiiuaua{sv})a(uxiausauaua{sv})a(uxuudu)ii'),
  ...
  Future<void> callApplyConfiguration(int serial, bool persistent,
      List<DBusStruct> crtcs, List<DBusStruct> outputs,
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    await callMethod(
        ...
        [
          ...
          DBusArray(
              DBusSignature('(uiiiuaua{sv})'), crtcs.map((child) => child)),
          DBusArray(DBusSignature('(ua{sv})'), outputs.map((child) => child))
        ],
  ...
  Future<int> callChangeBacklight(int serial, int output, int value,
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod(
        'org.gnome.Mutter.DisplayConfig',
        'ChangeBacklight',
        [DBusUint32(serial), DBusUint32(output), DBusInt32(value)],
  ...
  Future<List<DBusValue>> callGetCrtcGamma(int serial, int crtc,
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod('org.gnome.Mutter.DisplayConfig',
        'GetCrtcGamma', [DBusUint32(serial), DBusUint32(crtc)],
        replySignature: DBusSignature('aqaqaq'),
        noAutoStart: noAutoStart,
        allowInteractiveAuthorization: allowInteractiveAuthorization);
    return result.returnValues;
  }
  ...
  Future<void> callSetCrtcGamma(
      int serial, int crtc, List<int> red, List<int> green, List<int> blue,
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    await callMethod(
        'org.gnome.Mutter.DisplayConfig',
        ...
        [
          DBusUint32(serial),
          DBusUint32(crtc),
          DBusArray.uint16(red),
          DBusArray.uint16(green),
          DBusArray.uint16(blue)
        ],
        replySignature: DBusSignature(''),
        noAutoStart: noAutoStart,
        allowInteractiveAuthorization: allowInteractiveAuthorization);
  ...
  Future<List<DBusValue>> callGetCurrentState(
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    var result = await callMethod(
  ...
  Future<void> callApplyMonitorsConfig(int serial, int method,
      List<DBusStruct> logical_monitors, Map<String, DBusValue> properties,
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    await callMethod(
        'org.gnome.Mutter.DisplayConfig',
        ...
        [
          DBusUint32(serial),
          DBusUint32(method),
          DBusArray(DBusSignature('(iiduba(ssa{sv}))'),
              logical_monitors.map((child) => child)),
  ...
  Future<void> callSetOutputCTM(int serial, int output, DBusStruct ctm,
      {bool noAutoStart = false,
      bool allowInteractiveAuthorization = false}) async {
    await callMethod('org.gnome.Mutter.DisplayConfig', 'SetOutputCTM',
        [DBusUint32(serial), DBusUint32(output), ctm],
        replySignature: DBusSignature(''),
        noAutoStart: noAutoStart,
        allowInteractiveAuthorization: allowInteractiveAuthorization);
  }
